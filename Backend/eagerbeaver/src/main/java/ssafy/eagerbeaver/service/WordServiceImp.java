...
public class WordServiceImp implements WordService {
	...
	// 랜덤 용어 담는 배열
	private final WordDto[] result = new WordDto[3];
	...
	public void choiceRandomWord() {
		...
		for (int idx2 = 0; idx2 < 3; idx2++) {
			result[idx2] = WordDto.builder()
				.id(wordRepository.findById(randomId[idx2]).get().getId())
				.content(wordRepository.findById(randomId[idx2]).get().getContent())
				.meaning(wordRepository.findById(randomId[idx2]).get().getMeaning())
				.build();
		}
